  ...
  def elastic_flow_step(beta_ti_in, iis_in, S_F, S_C, S_0, m, reason):
    ...
    for k in range(TAYLOR_COEFFS):
      ...
      for k1 in range(1,k+2):
        Omega1_k1=-(1-ALPHA)*np.diag(iis_0[k1-1,:]).dot(Ss)
        ...
        Omega2_k1*=(1-ALPHA)**2/2
        Omega1_ks.append(Omega1_k1)
        Omega2_ks.append(Omega2_k1)
        Omega_ks.append(Omega1_k1+Omega2_k1)
      
      if USE_OMEGA2:
        ck1=-Ss.dot(get_expm_der(Omega_ks)).dot(beta_hat.reshape((-1,1))-ALPHA/(1-ALPHA)*Ss_inv.dot(my_sign(g_ti))-beta_ti.reshape((-1,1))) #g^(k+1)(t_i)
      else:
        ck1=-Ss.dot(get_expm_der(Omega1_ks)).dot(beta_hat.reshape((-1,1))-ALPHA/(1-ALPHA)*Ss_inv.dot(my_sign(g_ti))-beta_ti.reshape((-1,1))) #g^(k+1)(t_i)

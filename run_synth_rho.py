  ...
  for alg in algs:
    ...
    for rho1 in rho1s:
      ...
      for rho2 in rho2s:
        ...
        data_dict[p][alg][rho1][rho2]={'sens': get_sens(beta, beta_star), 'spec': get_spec(beta, beta_star), 'pred_err': get_pred_err(beta, beta_star, X_test), 'est_err': get_est_err(beta, beta_star), 'time_spent': time_spent, 'snr': snr}

fi=open('data/synth_rho_'+str(seed)+'.pkl','wb')
